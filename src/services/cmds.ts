...
export async function calcuProxies(): Promise<{
  ...
  >((acc, each) => {
    if (each?.name !== 'GLOBAL' && each?.all) {
      acc.push({
        ...each,
          all: each.all!.map((item: string) => generateItem(item)),
      })
    }
  ...
  const _global = {
    ...global,
    all: global?.all?.map((item: string) => generateItem(item)) || [],
  }
...
export async function startVpn() {
  return invoke<VpnStatus>('start_vpn')
}

export async function stopVpn() {
  return invoke<VpnStatus>('stop_vpn')
}
